...
void setup() {
  ...
  modbus.onData([](uint8_t serverAddress, esp32Modbus::FunctionCode fc, uint16_t address, uint8_t* data, size_t length) {
     ...
     String SendingData[length+1];
     SendingData[0] = String(Register-1);
     switch (Register-1){
      case State:
        StateValue = (256*data[0]) + data[1];
        SendingData[1] = String(StateValue);
      ...
      case ProductNo:
        ...
        SendingData[1] = String(ProductNoValue);
        for(int i = 0; i<length+1; i++){
          Serial.print(SendingData[i]);
        }
        break;
      case AdditivType:
        ...
        SendingData[1] = String(AdditivTypeValue);
        for(int i = 0; i<length+1; i++){
          Serial.print(SendingData[i]);
        }
        break;
